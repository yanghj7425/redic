package com.robert.redis.redic.core;
...
public class Redic extends Jedis {
    ...
    public Redic addNode(String nodeConnStr) {
        ...
        //check length odf nodes
        if (nodes.length == 0) {
            throw new IllegalArgumentException("nodeConnStr cannot be null");
        }
        ...
        if (nodes.length == 1) {
            ...
            //if length of nodes >1,slave from 1 to length
            slaveList = Arrays.asList(Arrays.copyOfRange(nodes, 1, nodes.length));
        }

        return addNode(nodes[0], slaveList);
    }
